...
---
module: contactgroup
short_description: Manage contact groups in Nagios XI
description:
  - Create, update, and delete C(contactgroup) objects through the Nagios XI
    REST API (C(config/contactgroup) endpoints of the Core Config Manager).
...
options:
  contactgroup_name:
    description:
      - Short name of the contact group.
    type: str
    required: true
  state:
    description:
...
EXAMPLES = r'''
- name: Ensure contactgroup exists
  community.nagios_cs.contactgroup:
    url: https://nagios.example.com
    api_key: "{{ nagios_api_key }}"
    contactgroup_name: my-contactgroup-name
    properties:
      alias: "On-call"
      members: "jane,john"
- name: Remove contactgroup
  community.nagios_cs.contactgroup:
    url: https://nagios.example.com
    api_key: "{{ nagios_api_key }}"
    contactgroup_name: my-contactgroup-name
...
def main():
    run_config_module('contactgroup')
