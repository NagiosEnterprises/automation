...
DOCUMENTATION = r'''
---
module: serviceescalation
short_description: Manage service escalations in Nagios XI
description:
  - Create, update, and delete C(serviceescalation) objects through the Nagios XI
    REST API (C(config/serviceescalation) endpoints of the Core Config Manager).
...
options:
  ...
  service_description:
    description:
      - Service the escalation applies to.
    type: str
    required: true
  state:
    description:
...
EXAMPLES = r'''
- name: Ensure serviceescalation exists
...
def main():
    run_config_module('serviceescalation')
