...
---
module: servicegroup
short_description: Manage service groups in Nagios XI
description:
  - Create, update, and delete C(servicegroup) objects through the Nagios XI
    REST API (C(config/servicegroup) endpoints of the Core Config Manager).
...
options:
  servicegroup_name:
    description:
      - Short name of the service group.
    type: str
    required: true
  state:
    description:
...
def main():
    run_config_module('servicegroup')
